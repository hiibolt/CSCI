...
{
	if ( this->q_capacity == 0 ) {
		this->reserve(1);
	}
	if ( this->q_size == this->q_capacity ) {
		this->reserve(this->q_capacity * 2);
	}
	this->q_array[this->q_size] = value;
	this->q_size ++;
}
...
void myqueue::pop()
{
	for ( size_t i = 1; i < this->q_size; i++ ) {
		this->q_array[i - 1] = this->q_array[i];
	}
	this->q_size --;
}
...
{
	return this->q_array[this->q_size - 1];
}
