...
double getLesserValue( string prompt, double upper_bound ) {
    double ret;

    cout << prompt << " (must be less than " << upper_bound << "): ";
    cin >> ret;

    while ( ret >= upper_bound ) {
        cout << "Error: the value must be less than " << upper_bound << ". Try again: ";
        cin >> ret;
    }
